    ...
    return (
        <View style={{ display: menuOpen ? 'flex' : 'none', position: 'absolute', backgroundColor: '#aaa', padding: 8, width: 150, top: 100, right: 0, borderTopLeftRadius: 6, borderBottomLeftRadius: 6, zIndex: 500, }}>
            ...
            <TouchableOpacity onPress={() => remove()}>
                <View style={styles.container}>
                    <Entypo name="trash" size={18} color='#111' style={styles.icon} />
                    <TextComp style={{ fontSize: 16 }}>Delete</TextComp>
                </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={goToDetail}>
                <View style={styles.container}>
                    <Entypo name="dots-three-horizontal" size={18} color='#111' style={styles.icon} />
                    <TextComp style={{ fontSize: 16 }}>More</TextComp>
                </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => setMenuOpen(false)}>
                <View style={styles.container}>
                    <AntDesign name="close" size={18} color='#111' style={[styles.icon, { color: theme.red }]} />
                    <TextComp style={{ fontSize: 16, color: theme.red }}>Close</TextComp>
                </View>
            </TouchableOpacity>
        </View>
    )
